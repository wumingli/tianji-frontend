...
$(function (){
        ...
        if (!bFail){
            ...
            if (obj.attr('type') === 'password'){
                if(obj.parents('form').attr('id') === 'tianji-pop-login-form'){
                    obj.after('<div class="error">请输入密码</div>');
                } else {
    ...
    $('#tianji-popup-login .tianji-submit-btn').live('click', function (){
        ...
        if ($('[name="account[email_or_mobile]"]').next('div.error').length == 0){
            $('[name="account[email_or_mobile]"]').trigger('blur');
        }
    ...
    $('input:[name="account[email_or_mobile]"]').live('blur',function (){
        var $this = $(this),
            thisVal = $.trim($(this).val());
        if (thisVal !== '' && $this.next('div.error').length == 0){
            __$.ajax({
                    ...
                    200: function (){
                        ...
                        if ($this.attr('id') === 'tianji-login-email_or_mobile'){
                            if ($this.next('div.error').length !== 0){
                                $this.next('div.error').html('该帐号不存在，请更换帐号 或 <a href="javascript:void(0);" class="fblue reg-now">注册</a>');
                            } else {
                                $this.after('<div class="error">该帐号不存在，请更换帐号 或 <a href="javascript:void(0);" class="fblue reg-now">注册</a></div>');
                            }
                        }
                    },
                    201: function (){
    ...
    $('.pop-login input:[name="account[email_or_mobile]"]').live('blur',function (){
        ...
        if (thisVal !== '' && $this.next('div.error').length == 0){
            __$.ajax({
                ...
                statusCode:{
                    200: function (){
                        $this.prev('span.inputTip').hide().end().val($this.val()).next('div.error').remove();
                        if ($this.next('div.error').length !== 0){
                            $this.next('div.error').html('该帐号不存在，请更换帐号 或 <a href="javascript:void(0);" class="fblue reg-now">注册</a>');
                        } else {
                            $this.after('<div class="error">该帐号不存在，请更换帐号 或 <a href="javascript:void(0);" class="fblue reg-now">注册</a></div>');
    ...
    //注册检查帐号是否存在
    $('.pop-reg input:[name="account[email_or_mobile]"]').live('blur',function (){
        var $this = $(this),
            thisVal = $.trim($(this).val());
        if (thisVal !== '' && $this.next('div.error').length == 0){
            __$.ajax({
