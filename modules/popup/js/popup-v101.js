...
(function($) {
    ...
    function fnPopType(cfg, callback) {
        ...
        switch (content['type']) {
                    ...
                    $.ajax(content['dataUrl'], {
                        ...
                        success: function(data) {
                            ...
                            if ($container.children('li').length > 0) {
                                $container.children('li').wrapAll('<ul class="popup_list" />');
                                //$container.append(strPage);
                                if (cfg.showPage) {
                                    $container.find('.popup_page').show();
                                } else {
                                    $container.find('.popup_page').hide();
                                }
                                //$container.append(strPreNext);
                                if (cfg.showPreNext) {
                                    $container.find('.popup_page_btn').show();
                                } else {
                                    $container.find('.popup_page_btn').hide();
                                }
                            }
                            callback && callback();
                            autoChangeTitle(cfg);
            ...
            case 'userDefined':
                ...
                $container.find('.popup_loading').remove();
                $container.append(html);
                if ($container.children('li').length > 0) {
                    $container.children('li').wrapAll('<ul class="popup_list" />');
                    $container.append(strPage);
    ...
    var tianjiPopup = function(cfg) {
        ...
        //是否启用Cookie
        if (cfg.cookie && document.cookie.indexOf(cfg.name + '_remembered=true') != -1) {
            return;
        }
        ...
        fnPopType(cfg, function() {
            ...
            cfg.showPreNext && $('.popup_page_btn,.popup_next_btn').show();
            cfg.showPage && $('.popup_page').show();
            //}, 500);
            //if (cfg.content && (cfg.content.type === 'focus' || cfg.content.type === 'ajaxData')) {
            //动态计算页数
            var strPage = '';
            var itemLen = $('#tianjiPopupContainer .popup_list li').length;
            if (itemLen > 0) {
                ...
                $('#tianjiPopupContainer .popup_container .popup_page, #tianjiPopupContainer .popup_container .popup_page_btn').hide();
            }
            //绑定播放事件
            ...
            //}
            $('#tianjiPopupContainer .popup_list li').width(cfg.boxWidth);
        ...
        /*if (cfg.closeWhenClickPage) {
            $('#tianjiPopupContainer_shandow').on('click', closeBox);
            $('#tianjiPopupContainer').on('click', function(e) {
                e.cancelBubble = true;
                e.stopPropagation();
            });
        } 
    ...
    window.tianjiPopup = tianjiPopup;
})(jQuery);